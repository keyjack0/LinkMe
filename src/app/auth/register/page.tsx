...
import { signUp, signInWithGoogle, signInWithGithub } from '@/lib/actions/auth.actions'
...
export default function RegisterPage() {
  ...
  const usernameValid = username.length >= 3 && isValidUsername(username)
  const usernameInvalid = username.length > 0 && !usernameValid

  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault()
    if (!usernameValid) { toast.error('Username tidak valid'); return }
    setLoading(true)
  ...
  return (
    <div className="card shadow-xl shadow-black/5">
      <div className="mb-8">
        <h1 className="font-display text-2xl font-bold text-ink mb-1">Buat akun LinkMe</h1>
        <p className="text-sm text-ink-muted">Gratis selamanya, setup dalam 2 menit</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-ink mb-1.5">
            Username <span className="text-ink-muted font-normal">(linkme.id/username-mu)</span>
          </label>
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-ink-muted">linkme.id/</span>
            <input
              name="username"
              type="text"
              required
              ...
              className={`input-base pl-[88px] pr-9 ${usernameInvalid ? 'border-red-400 focus:ring-red-300' : ''}`}
            />
            {username.length > 0 && (
          ...
          {usernameInvalid && (
            <p className="text-xs text-red-500 mt-1">Gunakan huruf kecil, angka, titik, atau garis bawah (min 3 karakter)</p>
          )}
        </div>
        ...
        <button type="submit" disabled={loading || !usernameValid} className="btn-primary w-full py-3">
        ...
        <p className="text-xs text-center text-ink-muted">
          ...
          <Link href="/terms" className="text-brand hover:underline">Syarat & Ketentuan</Link>{' '}
          dan{' '}
          <Link href="/privacy" className="text-brand hover:underline">Kebijakan Privasi</Link> kami.
        </p>
      </form>

      <p className="text-center text-sm text-ink-muted mt-6">
        Sudah punya akun?{' '}
        <Link href="/auth/login" className="text-brand font-medium hover:underline">Masuk</Link>
      </p>
    </div>
  )
}
